...
/**
 * Sorts an array by selecting the pivot value to be any random value of the array
 * @author Yazdan Basir & Jannat-ul-Ferdous
 */
public class QuickSortRandom <T extends Comparable <? super T>> extends QuickSorter <T> {
    int start;
    int end;
    ...
    public int partition (T[] a, int fst, int lst) {
        Random random = new Random(System.currentTimeMillis()); // Creates a new instance of random

        int randomPivot = (random.nextInt(lst - fst)) + fst; // Generates a random number in the range between first and last

        T pivot = a[randomPivot]; // Assigns that random index to be the pivot
